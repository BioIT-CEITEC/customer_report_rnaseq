...
col.count <- names(multiqc_star) %!like% "percent" &
  names(multiqc_star) %!like% "_rate" &
  names(multiqc_star) %!like% "_length" &
  names(multiqc_star) %!like% "_splices" &
  names(multiqc_star) %!like% "condition" &
  names(multiqc_star) %!like% "total_"

multiqc_star_count <- multiqc_star[, ..col.count]
multiqc_star_count <- melt(multiqc_star_count, id.vars = "Sample")
multiqc_star_count <- multiqc_star_count[, variable := factor(variable, levels = c("uniquely_mapped","multimapped","multimapped_toomany","unmapped_mismatches","unmapped_tooshort","unmapped_other"))]
...
star.perc.not0 <- multiqc_star_perc[, .(sum = sum(value)), by = variable][sum > 0,]$variable
multiqc_star_perc <- multiqc_star_perc[variable %in% star.perc.not0,]
star.labcol.perc <- star.labcol[variable %in% star.perc.not0,]

star.count.not0 <- multiqc_star_count[, .(sum = sum(value)), by = variable][sum > 0,]$variable
multiqc_star_count <- multiqc_star_count[variable %in% star.count.not0,]
...
plotly.multiqc_star_perc <- ggplotly(plot.multiqc_star_perc) %>% layout(legend = list(orientation = 'h', y=-0.3))
...
plotly.multiqc_star_count <- ggplotly(plot.multiqc_star_count) %>% layout(legend = list(orientation = 'h', y=-0.3))
...
##### Number of reads
```{r star-map-count, results='asis', echo=FALSE, fig.cap="Percentages and distribution of different mapping types. For more details please see the STAR aligner manual.", fig.align="center", out.width="90%", fig.show="hold", dev="svg", fig.width=7, fig.height = custom_height1}
...
- All samples had majority of reads ~`r picard.mrna`% counted over bases as well as ~`r rseqc.mrna`% counted over reads mapped within coding regions (`r paste0("Figures \\@ref(fig:picard-map-loc-pct) and \\@ref(fig:rseqc-map-loc-pct)")`).
...
plot.rsem.assign <- ggplot(rsem.assign.tabl, aes(Sample, value, fill = factor(variable, levels = c("aligned.unique","aligned.multi","filter","unaligned")))) +
  ...
  plotly.rsem.assign <- ggplotly(plot.rsem.assign) %>% layout(legend = list(orientation = 'h', y=-0.3))
