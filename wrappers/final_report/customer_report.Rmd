...
- Read counts for coding genes (mostly protein-coding genes) distribution is depicted in the following Figure(s).
- Post-normalization counts (~normalized expressions) can be controlled for unusual remaining differences in expression level between samples.
- If the normalized expression is not similar between samples, it might be needed to sub-sample or remove sample(s) with too high expression difference.
...
norm.plot<-function(analysis_type = analysis_type, DE = DE){
  ...
  for(j in 1:length(analysis_type)){
    ...
    for(i in 1:length(ppnc.files)){
      output<-c(output,cat("<div align='center'><img src=",ppnc.files[i]," width='80%'/></div>"))
      output<-c(output,cat("\n\n"))
    }
  }
  #output<-c(output,cat("## {-}"))
...
norm.plot(analysis_type, DE)

```

### {-}

Number of reads assigned to coding genes for all the samples before normalization (raw counts) and after normalization (normalized counts).
...
- Samples from different condition clustering together might indicate a reduce effect coming from the tested treatment which often leads to low differentially expressed gene identification.

```{r de-pca, echo=FALSE, out.width="80%", results="asis", dev="svg"}

pca_heatmap.plot<-function(analysis_type = analysis_type, DE = DE){
  output<-cat("")
  for(j in 1:length(analysis_type)){
    output<-c(output,cat("#### ",analysis_type[j],"\n"))
    ...
    if(config_json$conditions_to_compare[1] == "all"){
      ...
      sample.sample.files[, path_pca:=ifelse(file.exists(paste0("../results/",DE[j],"/",comp,"/all/sample_to_sample_PCA_batch2.svg")), paste0("../results/",DE[j],"/",comp,"/all/sample_to_sample_PCA_batch2.svg"), paste0("../results/",DE[j],"/",comp,"/all/sample_to_sample_PCA2.svg"))]
      sample.sample.files[, path_heatmap:=ifelse(file.exists(paste0("../results/",DE[j],"/",comp,"/all/heatmaps_samples_log_batch.png")), paste0("../results/",DE[j],"/",comp,"/all/heatmaps_samples_log_batch.png"), paste0("../results/",DE[j],"/",comp,"/all/heatmaps_samples_log.png"))]
    }else{
      sample.sample.files <- data.table(comp = comparison)
      sample.sample.files[, path_pca:=ifelse(file.exists(paste0("../results/",DE[j],"/",comp,"/all/sample_to_sample_PCA_batch2.svg")), paste0("../results/",DE[j],"/",comp,"/all/sample_to_sample_PCA_batch2.svg"), paste0("../results/",DE[j],"/",comp,"/all/sample_to_sample_PCA2.svg"))]
      sample.sample.files[, path_heatmap:=ifelse(file.exists(paste0("../results/",DE[j],"/",comp,"/all/heatmaps_samples_log_batch.png")), paste0("../results/",DE[j],"/",comparison,"/all/heatmaps_samples_log_batch.png"), paste0("../results/",DE[j],"/",comp,"/all/heatmaps_samples_log.png"))]
    }

    for(i in 1:length(sample.sample.files$path_pca)){
      output<-c(output,cat("<div align='center'><img src=",sample.sample.files$path_pca[i]," width='50%' float='left'/><img src=",sample.sample.files$path_heatmap[i]," width='50%' float='right'/></div>"))
      output<-c(output,cat("\n\n"))
    }
  }
  #output<-c(output,cat("\n### {-}"))
...
pca_heatmap.plot(analysis_type, DE)

```

### {-}

PCA (first two components) visualization of all samples (DESeq2 VST normalization).
Heatmap of samples clustered by gene expression (DESeq2 log2 normalization).
