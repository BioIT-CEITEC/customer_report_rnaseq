...
`r if(var.rsem==1){paste0("  - The library preparation kit strand-specificity is: **",tolower(strandness),"**, and only reads in this strand should be considered.")}`
...
- All samples had a `r intron.count` content of intronic reads ~`r picard.intron`% counted over bases as well as ~`r rseqc.intron`% counted over reads which is expected for polyA selection library protocols
...
- Please note that for visualization purposes we had to set a predefined cut-off value for some of the parameters (adj.p-value is set to 0.05 and log2FC is set to -1/+1)
...
- Volcano plot visualization displays log2FC (x-axis) and adj.p-value (y-axis).
...
```{r, results="asis", echo=FALSE}
capt <- paste0("Summary or number of DE genes based on default cut-off values from all three DE calculations. The first two columns are counts with both adj.p-value and log2FC cut-off, the other two columns are only with adj.p-value cut-off. DESeq2 results, DESeq2 results without independent filtering (noIndFilt) and edgeR results are shown.")
```
...
ftabl.de.de.all <- flextable(tabl.de.de.all[,.(Comparison, Analysis, "Up (adj.pval & log2FC)"=up_pval_logfc, "Down (adj.pval & log2FC)"=down_pval_logfc,"Up (adj.pval)"=up_pval,"Down (adj.pval)"=down_pval)]) # change the table to a flextable object
...
```{r de-top-de, fig.cap = paste0("Heatmap of maximum top 20 differentially expressed genes (DESeq2 results with independent filtering, adj.p-value < 0.05, log2FC >= +/-1)."), echo=FALSE, out.width="80%", fig.align="center", results="asis", fig.show="hold", dev="svg"}
...
- Commonly used statistical values, such as log2FC and adj.p-value, should help you to confirm the hypothesis, results or the observations and not lead the discovery itself.
- If you decide to go for some cut-off values, do not blindly select only genes above the thresholds!
  ...
  - The similar thing applies to log2FC.
  - In the case of log2FC, we also have to consider the effect of the expression of individual genes. For example, the frequently used cut-off value of 1 (2x fold-change): A gene with expression 10 in one group and 5 in other group results in log2FC 1 and would pass the cut-off. The overall expression of the gene is negligible and most likely the change does not have any biological effect. Another gene has an expression of 10000 in one group and 5001 in another group. This gene would not pass the cut-off value but most likely the change will have some kind of biological effect.
  - Sometimes small log2FC might be more important than high log2FC and the same applies to adj.p-value.
...
- **all_sig_genes_normCounts.pdf** – significantly DEG (here, “default cut-off” adj.p-value 0.05 and log2FC 1 is considered) expression visualization. Selection of DE genes is based on the particular comparison but all conditions/samples are plotted. This is based on DESeq2 results.
...
- **DESeq2_de_genes_check.txt** and **DESeq2_de_genes_check_noIndFilt.txt** – basic summary of differential expression. “Default” adj.p-value of 0.05 and log2FC of 1 are used. The upper part of the table considers only adj.p-value cut-off value, the lower part considers adj.p-value and log2FC. This servers only for a  demonstration of the possible effect, not as a final result.
...
- **overlap_DESeq2_edgeR_de.tsv** and **overlap_DESeq2_edgeR_venn.pdf** – overlap between DE genes between DESeq2 and edgeR calculation. In default, genes with adj. p-value <0.05 and log2FC +/- 1 are included. Columns DESeq2 and edgeR contain values TRUE or FALSE and this represents whether the resulting gene was included in the results by an individual tool.
...
`r if(var.rsem==1){"### RSEM_expression"}`
`r if(var.rsem==1){"- Gene and transcript expression for each sample."}`
`r if(var.rsem==1){"- The normalized expressions are summarized in TPM column."}`

`r if(var.rsem==1){"#### Files"}`
`r if(var.rsem==1){"- .genes.results.tsv - gene expression estimates."}`
`r if(var.rsem==1){"- .isoforms.results.tsv - transcript expression estimates."}`
...
- Input for gene ontology (GO) testing is usually a list of differentially expressed genes.
  - You can either use filtering by some cut-off value(s) or select top X genes sorted by adj.p-value/log2FC/...
...
- You do NOT subsample your results but you take a whole list of gene expression results and sort it according to specified criteria (log2FC, adj.p-value, ...)
  - In the case of log2FC, you have the most positively changed genes on the top and the most negatively changed genes at the bottom and non-changed genes are in the middle but they are still in the SAME list.
