...
volcano.ma.plots<-function(comparison=comparison, analysis_type = analysis_type){
  output<-cat("#### ",comparison[1]," {.tabset}","\n")
  for(j in 1:length(analysis_type$DE)){
    volcano.files <- grep(grep(grep(list.files(path=paste0("../",analysis_type$DE[j],"/",comparison[1],"/report_data/"),
                               pattern = "volcanoplot_", full.names = T), pattern="Filt", value = T, invert = T),
                               pattern="edgeR", value = T, invert = T), pattern=".png", value=T)
    ma.files <- grep(grep(grep(list.files(path=paste0("../",analysis_type$DE[j],"/",comparison[1],"/report_data/"),
  ...
  if(length(comparison)>1){
    for(i in 2:length(comparison)){
      output<-c(output,cat("#### ",comparison[i],"\n"))
      for(j in 1:length(analysis_type$DE)){
        volcano.files <- grep(grep(grep(list.files(path=paste0("../",analysis_type$DE[j],"/",comparison[i],"/report_data/"),
                               pattern = "volcanoplot_", full.names = T), pattern="Filt", value = T, invert = T),
                               pattern="edgeR", value = T, invert = T), pattern=".png", value=T)
        ma.files <- grep(grep(grep(list.files(path=paste0("../",analysis_type$DE[j],"/",comparison[i],"/report_data/"),
                               pattern = "MAplot_", full.names = T), pattern="Filt", value = T, invert = T),
                               pattern="edgeR", value = T, invert = T), pattern=".png", value=T)
...
no_null<-volcano.ma.plots(comparison, analysis_type)
...
top.de.plots<-function(comparison=comparison, analysis_type = analysis_type){
  output<-cat("#### ",comparison[1]," {.tabset}","\n")
  for(j in 1:length(analysis_type$DE)){
    top_de.files <- list.files(path=paste0("../",analysis_type$DE[j],"/",comparison[1],"/report_data/"),
  ...
  if(length(comparison)>1){
    for(i in 2:length(comparison)){
      output<-c(output,cat("#### ",comparison[i],"\n"))
      for(j in 1:length(analysis_type$DE)){
        top_de.files <- list.files(path=paste0("../",analysis_type$DE[j],"/",comparison[i],"/report_data/"),
                               pattern ="heatmap_selected_orderBaseMeanCluster.png", full.names = T)
...
no_null<-top.de.plots(comparison, analysis_type)
