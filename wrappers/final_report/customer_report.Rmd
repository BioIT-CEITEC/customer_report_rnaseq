...
analysis_type <- analysis_type[,DE := paste0("DE_",analysis)] # featureCount is usually run by default
...
organism <- config_json$organism[1]
assembly <- config_json$assembly[1]
release <- config_json$release[1]
...
tabl_sample <- fread(paste0(MQC_DATA,"multiqc_cutadapt.txt"), header=T)
tabl_sample[, Sample := gsub("_R.","",Sample)]
#if(is.null(tabl_sample$input_reads)==T){tabl_sample[,input_reads:=input_read_pairs]}
tabl_sample <- tabl_sample[,input_reads:=ifelse("r_processed" %in% names(tabl_sample),r_processed,pairs_processed),by=.(Sample)]
tabl_sample <- tabl_sample[,output_reads:=ifelse("r_written" %in% names(tabl_sample),r_written,pairs_written),by=.(Sample)]
...
fit_to_width(autofit(qflextable(tabl_sample[, .(Sample, condition, "raw reads" = input_reads, "preprocessed reads" = output_reads)])),max_width = 9.1)
```

## Analysis
- Raw reads were quality checked (**FastQC, MultiQC, minion, swan**), preprocessed (**Cutadapt, FastQC, MultiQC**) and mapped (**STAR, Samtools, MultiQC**) to the reference genome with gene annotation (**genome version: `r paste0(assembly," ",release)`**).
- Mapped reads were counted and summarized to genes with:
`r if(config_json$featureCount[1] == TRUE){"  - **featureCounts**."}`
...
if(file.exists(paste0(MQC_DATA,"mqc_fastqc_adapter_content_plot_1.txt"))){
 adaptor_max <- fread(paste0(MQC_DATA,"mqc_fastqc_adapter_content_plot_1.txt"), header=T, fill=T)
 adaptor_max[is.na(adaptor_max)]<-0
 setnames(adaptor_max, "V1", "Sample")
 adaptor_max <- melt(adaptor_max, id.vars = c("Sample"), value.name = "plot.adaptor.max")
 adaptor_max <- adaptor_max[Sample != ""]
 adaptor_max <- adaptor_max[Sample %!like% "raw"]
}else{
...
- The `r paired` reads were aligned to `r species` reference genome with the help of the gene annotation (`r paste0(assembly," ",release)`) using STAR aligner.
...
if(config_json$display_fastq_screen_RNA[1] == TRUE){
  ...
  fastq.screen.files <- fastq.screen.files[Sample %like% paste(config_json$full_name, collapse="|"),]
...
if(config_json$display_RSeQC_RNA[1]==TRUE){
  ...
  rseqc.metric.tbl <- rseqc.metric.tbl[, `:=` (true_tss_up_5kb_tag_count = `tss_up_1kb-5kb_tag_count`,
                                               true_tss_up_10kb_tag_count = `tss_up_5kb-10kb_tag_count`,
                                               true_tes_down_5kb_tag_count = `tes_down_1kb-5kb_tag_count`,
                                               true_tes_down_10kb_tag_count = `tes_down_5kb-10kb_tag_count`)]
...
`r if(config_json$RSEM[1] == TRUE | config_json$featureCount[1] == TRUE | config_json$salmon_align[1] == TRUE | config_json$salmon_map[1] == TRUE | config_json$kallisto[1] == TRUE){"### Read count assignment to genes"}`
```{r map-assign, echo=FALSE}
if(config_json$RSEM[1] == TRUE){
  rsem.assign <- fread(paste0(MQC_DATA,"multiqc_rsem.txt"))
  rsem.assign <- rsem.assign[,Sample:=as.character(Sample)]
...
if(config_json$featureCount[1] == TRUE){
  fc.assign <- fread(paste0(MQC_DATA,"multiqc_featureCounts.txt"))
  fc.assign <- fc.assign[,Sample:=as.character(Sample)]
  fc.assign <- fc.assign[,count_type:=unlist(strsplit(Sample,split="_",fixed=T))[1],by=.(Sample)]
  fc.assign <- fc.assign[,Sample:=gsub("exon_","",
                                       gsub("gene_","",
                                            gsub("transcript_","",
                                                 gsub("3pUTR_","",
                                                      gsub("5pUTR_","",Sample))))), by=.(Sample)]
  fc.assign <- merge(config_json[, .(Sample = full_name, condition)], fc.assign, by="Sample")
  setorder(fc.assign, condition, Sample)

  fc.assign <- melt(fc.assign[, percent_assigned:=NULL], id.vars = c("Sample","condition","count_type","Total"))
  fc.assign <- fc.assign[, percentage := round(100*value/Total, 2), by = .(Sample,count_type)]

  fc.values<-function(fc.assign, count_type="exon"){
    fc.assign.min <- round(min(fc.assign[count_type == count_type & variable=="Assigned",value]/1E6),0)
    fc.assign.max <- round(max(fc.assign[count_type == count_type & variable=="Assigned",value]/1E6),0)
    fc.assign.min.perc <- round(min(fc.assign[count_type == count_type & variable=="Assigned",percentage]),0)
    fc.assign.max.perc <- round(max(fc.assign[count_type == count_type & variable=="Assigned",percentage]),0)
    fc.assign.mean.perc <- round(mean(fc.assign[count_type == count_type & variable=="Assigned",percentage]),2)
    return(list(fc.assign.min,fc.assign.max,fc.assign.min.perc,fc.assign.max.perc,fc.assign.mean.perc))
  }

  if("exon" %in% fc.assign$count_type){
    fc.val<-fc.values(fc.assign, count_type="exon")
  }else if("exon" %!in% fc.assign$count_type & "gene" %in% fc.assign$count_type){
    fc.val<-fc.values(fc.assign, count_type="gene")
  }else if("exon" %!in% fc.assign$count_type & "gene" %!in% fc.assign$count_type & "transcript" %in% fc.assign$count_type){
    fc.val<-fc.values(fc.assign, count_type="transcript")
  }

  fc.assign.min <- fc.val[1]
  fc.assign.max <- fc.val[2]
  fc.assign.min.perc <- fc.val[3]
  fc.assign.max.perc <- fc.val[4]
  fc.assign.mean.perc <- fc.val[5]
...
`r if(config_json$featureCount[1] == TRUE){paste0("#### Assignment featureCounts {.tabset}")}`

`r if(config_json$featureCount[1] == TRUE){paste0("- Read count assignment from **featureCounts** for RNA-Seq is ",fc.assign.perc," (between ",fc.assign.min.perc,"% and ",fc.assign.max.perc,"%, calculated only from uniquely mapped reads).")}`
`r if(config_json$featureCount[1] == TRUE){paste0("  - Number of assigned reads is above limits of the recommended number of reads (~",fc.assign.min,"-",fc.assign.max,"M).")}`
`r if(config_json$featureCount[1] == TRUE){paste0("  - Read assignment rates are summarized in Table \\@ref(tab:fc-assign-tab) and Figures \\@ref(fig:plotly-fc-assign-pct) to \\@ref(fig:plotly-fc-assign-count).")}`
...
`r if(config_json$featureCount[1] == TRUE){paste0("##### Percentages (counted over mapping)")}`
...
if(config_json$featureCount[1] == TRUE){plotly.fc.assign.pct}
```

`r if(config_json$featureCount[1] == TRUE){paste0("##### Percentages (counted over reads)")}`
...
if(config_json$featureCount[1] == TRUE){plotly.fc.reassign.pct}
```

`r if(config_json$featureCount[1] == TRUE){paste0("##### Number of Reads")}`
...
if(config_json$featureCount[1] == TRUE){plotly.fc.assign.count}
```

`r if(config_json$featureCount[1] == TRUE){paste0("#### {-}")}`
...
### IGV
- Reference genomes and annotations for `r paste0(assembly," ",release)` are available to download on [Ensembl](https://www.ensembl.org/index.html) website.
