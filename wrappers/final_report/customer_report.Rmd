...
analysis_type <- paste0("DE_",unlist(strsplit(unique(config_json$analysis_type),split="\\|")))
...
plotly.rseqc.infer.exp <- ggplotly(plot.rseqc.infer.exp) %>% layout(legend = list(orientation = 'h', y=-0.3))
...
`r if(config_json$salmon[1] == TRUE){"  - **Salmon**."}`
...
plotly.rrna.p <- ggplotly(rrna.p) %>% layout(legend = list(orientation = 'h', y=-0.3))
...
rseqc.metric.tbl.pct <- cbind(rseqc.metric[,1], rseqc.metric[, ..rseqc.pct])
#rseqc.metric.tbl <- rseqc.metric.tbl[, other_intergenic_tag_pct := 100-rowSums(.SD), .SDcols = 2:11]
rseqc.metric.tbl.pct <- rseqc.metric.tbl.pct[, `:=` (true_tss_up_5kb_tag_pct = tss_up_5kb_tag_pct - tss_up_1kb_tag_pct,
...
rseqc.metric.tbl.pct <- rseqc.metric.tbl.pct[, `:=` (tss_up_5kb_tag_pct = NULL,
...
rseqc.metric.tbl.count <- cbind(rseqc.metric[,1], rseqc.metric[, ..rseqc.count])
#rseqc.metric.tbl <- rseqc.metric.tbl[, other_intergenic_tag_pct := 100-rowSums(.SD), .SDcols = 2:11]
rseqc.metric.tbl.count <- rseqc.metric.tbl.count[, `:=` (true_tss_up_5kb_tag_count = tss_up_5kb_tag_count - tss_up_1kb_tag_count,
...
rseqc.metric.tbl.count <- rseqc.metric.tbl.count[, `:=` (tss_up_5kb_tag_count = NULL,
...
picard.metric.tbl.count <- picard.metric[, .(Sample, Coding = CODING_BASES, UTR = UTR_BASES, Intronic = INTRONIC_BASES, Intergenic = INTERGENIC_BASES, not_aligned = PF_NOT_ALIGNED_BASES)]
picard.metric.tbl.count <- melt(picard.metric.tbl.count, id.vars = "Sample")
picard.metric.tbl.count <- picard.metric.tbl.count[, variable := factor(variable, levels = c("Coding","UTR","Intronic","Intergenic","not_aligned"))]

plot.picard.metric.count <- ggplot(picard.metric.tbl.count, aes(Sample, value, fill = variable)) +
  geom_bar(stat = "identity" , width = 0.8, position = position_stack(reverse = TRUE)) +
...
plotly.picard.metric.count <- ggplotly(plot.picard.metric.count) %>% layout(legend = list(orientation = 'h', y=-0.3))

picard.metric.tbl.perc <- picard.metric[, .(Sample, Coding = CODING_BASES/PF_BASES, UTR = UTR_BASES/PF_BASES, Intronic = INTRONIC_BASES/PF_BASES, Intergenic = INTERGENIC_BASES/PF_BASES, not_aligned = PF_NOT_ALIGNED_BASES/PF_BASES)]
picard.metric.tbl.perc <- melt(picard.metric.tbl.perc, id.vars = "Sample")
picard.metric.tbl.perc <- picard.metric.tbl.perc[, value := 100*value]
picard.metric.tbl.perc <- picard.metric.tbl.perc[, variable := factor(variable, levels = c("Coding","UTR","Intronic","Intergenic","not_aligned"))]

plot.picard.metric.perc <- ggplot(picard.metric.tbl.perc, aes(Sample, value, fill = variable)) +
  geom_bar(stat = "identity" , width = 0.8, position = position_stack(reverse = TRUE)) +
  scale_fill_manual(values = rseqc.palette, labels = c("Coding","UTR","Intronic","Intergenic","PF not aligned"), name = "") +
  coord_flip() +
  scale_y_continuous(name = "Percentages", breaks = seq(0,100,10)) +
...
plotly.picard.metric.perc <- ggplotly(plot.picard.metric.perc) %>% layout(legend = list(orientation = 'h', y=-0.3))

rseqc.metric.tbl.pct <- melt(rseqc.metric.tbl.pct, id.vars = "Sample")
rseqc.metric.tbl.pct <- rseqc.metric.tbl.pct[, `:=` (Sample = gsub(".RSeQC.*","",Sample), variable = gsub("_tag_pct","",variable))]
rseqc.metric.tbl.pct <- rseqc.metric.tbl.pct[variable == "cds_exons", variable := "CDS_Exons"]
rseqc.metric.tbl.pct <- rseqc.metric.tbl.pct[variable == "5_utr_exons", variable := "5'UTR_Exons"]
rseqc.metric.tbl.pct <- rseqc.metric.tbl.pct[variable == "3_utr_exons", variable := "3'UTR_Exons"]
rseqc.metric.tbl.pct <- rseqc.metric.tbl.pct[variable == "introns", variable := "Introns"]
rseqc.metric.tbl.pct <- rseqc.metric.tbl.pct[variable == "tss_up_1kb", variable := "TSS_up_1kb"]
rseqc.metric.tbl.pct <- rseqc.metric.tbl.pct[variable == "true_tss_up_5kb", variable := "TSS_up_5kb"]
rseqc.metric.tbl.pct <- rseqc.metric.tbl.pct[variable == "true_tss_up_10kb", variable := "TSS_up_10kb"]
rseqc.metric.tbl.pct <- rseqc.metric.tbl.pct[variable == "tes_down_1kb", variable := "TES_down_1kb"]
rseqc.metric.tbl.pct <- rseqc.metric.tbl.pct[variable == "true_tes_down_5kb", variable := "TES_down_5kb"]
rseqc.metric.tbl.pct <- rseqc.metric.tbl.pct[variable == "true_tes_down_10kb", variable := "TES_down_10kb"]
rseqc.metric.tbl.pct <- rseqc.metric.tbl.pct[variable == "other_intergenic", variable := "Other_intergenic"]
rseqc.metric.tbl.pct <- rseqc.metric.tbl.pct[,variable := factor(variable, levels = c("CDS_Exons","5'UTR_Exons","3'UTR_Exons","Introns","TSS_up_1kb","TSS_up_5kb","TSS_up_10kb","TES_down_1kb","TES_down_5kb","TES_down_10kb","Other_intergenic"))]

plot.rseqc.metric.pct <- ggplot(rseqc.metric.tbl.pct, aes(Sample, value, fill = variable)) +
  geom_bar(stat = "identity" , width = 0.8, position = position_stack(reverse = TRUE)) +
...
plotly.rseqc.metric.pct <- ggplotly(plot.rseqc.metric.pct) %>% layout(legend = list(orientation = 'h', y=-0.3))

rseqc.metric.tbl.count <- melt(rseqc.metric.tbl.count, id.vars = "Sample")
rseqc.metric.tbl.count <- rseqc.metric.tbl.count[, `:=` (Sample = gsub(".RSeQC.*","",Sample), variable = gsub("_tag_count","",variable))]
rseqc.metric.tbl.count <- rseqc.metric.tbl.count[variable == "cds_exons", variable := "CDS_Exons"]
rseqc.metric.tbl.count <- rseqc.metric.tbl.count[variable == "5_utr_exons", variable := "5'UTR_Exons"]
rseqc.metric.tbl.count <- rseqc.metric.tbl.count[variable == "3_utr_exons", variable := "3'UTR_Exons"]
rseqc.metric.tbl.count <- rseqc.metric.tbl.count[variable == "introns", variable := "Introns"]
rseqc.metric.tbl.count <- rseqc.metric.tbl.count[variable == "tss_up_1kb", variable := "TSS_up_1kb"]
rseqc.metric.tbl.count <- rseqc.metric.tbl.count[variable == "true_tss_up_5kb", variable := "TSS_up_5kb"]
rseqc.metric.tbl.count <- rseqc.metric.tbl.count[variable == "true_tss_up_10kb", variable := "TSS_up_10kb"]
rseqc.metric.tbl.count <- rseqc.metric.tbl.count[variable == "tes_down_1kb", variable := "TES_down_1kb"]
rseqc.metric.tbl.count <- rseqc.metric.tbl.count[variable == "true_tes_down_5kb", variable := "TES_down_5kb"]
rseqc.metric.tbl.count <- rseqc.metric.tbl.count[variable == "true_tes_down_10kb", variable := "TES_down_10kb"]
rseqc.metric.tbl.count <- rseqc.metric.tbl.count[variable == "other_intergenic", variable := "Other_intergenic"]
rseqc.metric.tbl.count <- rseqc.metric.tbl.count[,variable := factor(variable, levels = c("CDS_Exons","5'UTR_Exons","3'UTR_Exons","Introns","TSS_up_1kb","TSS_up_5kb","TSS_up_10kb","TES_down_1kb","TES_down_5kb","TES_down_10kb","Other_intergenic"))]

plot.rseqc.metric.count <- ggplot(rseqc.metric.tbl.count, aes(Sample, value, fill = variable)) +
  geom_bar(stat = "identity" , width = 0.8, position = position_stack(reverse = TRUE)) +
  scale_fill_manual(values = rseqc.palette, labels = c("CDS_Exons","5'UTR_Exons","3'UTR_Exons","Introns","TSS_up_1kb","TSS_up_5kb","TSS_up_10kb","TES_down_1kb","TES_down_5kb","TES_down_10kb","Other_intergenic"), name = "") +
  coord_flip() +
  xlab("") +
...
plotly.rseqc.metric.count <- ggplotly(plot.rseqc.metric.count) %>% layout(legend = list(orientation = 'h', y=-0.3))
...
- All samples had majority of reads ~`r picard.mrna`% counted over bases as well as ~`r rseqc.mrna`% counted over reads mapped within coding regions (Figure \@ref(fig:picard-map-loc-pct) & \@ref(fig:picard-map-loc-count) and \@ref(fig:rseqc-map-loc-pct) & \@ref(fig:rseqc-map-loc-count)).
- All samples are good quality and can be safely used for Differential expression analysis.

#### Picard metrics {.tabset}
...
plotly.picard.metric.count
```

#### {-}

#### RSeQC metrics {.tabset}
...
plotly.rseqc.infer.exp
```

### Read count assignment to genes
```{r analysis-assignment, echo=FALSE}
count.assign <- function(dt = fc.assign, count, percent){
  count.assign.min <- round(min(dt[,count]/1E6),0)
  count.assign.max <- round(max(dt[,count]/1E6),0)
  count.assign.mean.perc <- round(mean(dt[,percent]),0)
  count.assign.min.perc <- round(min(dt[,percent]),0)
  count.assign.max.perc <- round(max(dt[,percent]),0)

  if(count.assign.mean.perc >= 60){
    quali <- "very high"
  }else if(count.assign.mean.perc >= 40 & count.assign.mean.perc < 60){
    quali <- "high"
  }else if(count.assign.mean.perc >= 20 & count.assign.mean.perc < 40){
    quali <- "medium"
  }else if(count.assign.mean.perc >= 5 & count.assign.mean.perc < 20){
    quali <- "low"
  }else if(count.assign.mean.perc < 5){
    quali <- "very low"
  }
...
if(config_json[,RSEM][1] == TRUE){
  ...
  setnames(rsem.assign, c("Unique","Multi","Filtered","Unalignable"), c("aligned.unique", "aligned.multi", "filter", "unaligned"))
  rsem.assign.tabl <- rsem.assign[, .(Sample, Total, aligned.unique, aligned.multi, filter, unaligned)]
  rsem.assign.tabl <- melt(rsem.assign.tabl, id.vars = c("Sample","Total"))
  rsem.assign.tabl <- rsem.assign.tabl[, percentage:=round(100*value/Total, 2), by="Sample"]
  rsem.assign.tabl <- rsem.assign.tabl[, variable:=factor(variable, levels = c("aligned.unique", "aligned.multi", "filter", "unaligned"))]

  plot.rsem.assign.pct <- ggplot(rsem.assign.tabl, aes(Sample, percentage, fill = variable)) +
    geom_bar(stat = "identity" , width = 0.8, position = position_stack(reverse = TRUE)) +
  ...
  plotly.rsem.assign.pct <- ggplotly(plot.rsem.assign.pct) %>% layout(legend = list(orientation = 'h', y=-0.3))

  plot.rsem.assign.count <- ggplot(rsem.assign.tabl, aes(Sample, value, fill = variable)) +
    geom_bar(stat = "identity" , width = 0.8, position = position_stack(reverse = TRUE)) +
    scale_fill_manual(values = star.palette[c(1,4,5,6)], name = "", labels = c("Aligned uniquely to a gene","Aligned to multiple genes","Filtered due to too many alignments","Unalignable reads")) +
    coord_flip() +
    xlab("") +
  ...
  plotly.rsem.assign.count <- ggplotly(plot.rsem.assign.count) %>% layout(legend = list(orientation = 'h', y=-0.3))
...
fc.assign.avg.perc <- round(mean(fc.assign[,percent_assigned]),0)
...
plotly.fc.assign <- ggplotly(plot.fc.assign) %>% layout(legend = list(orientation = 'h', y=-0.3))
...
if(length(unique(pct$Sample)) == 1){
  ...
  plotly.mybar <- ggplotly(mybar) %>% layout(legend = list(orientation = 'h', y=-0.3))
