...
and the number of mapped reads varies between ~`r map.min`M for `r tabl_mapread[mapped == min(mapped),]$Sample` and ~`r map.max`M for `r tabl_mapread[mapped == max(mapped),]$Sample` (Table \@ref(tab:tabl-mapread), Figure \@ref(fig:star-map-perc) & \@ref(fig:star-map-count)).
...
pca_heatmap.plot<-function(analysis_type = analysis_type, DE = DE){
  ...
  for(j in 1:length(analysis_type)){
    ...
    for(i in 1:length(sample.sample.files$path_pca)){
      output<-c(output,cat("<div align='center'><figure style='width:50%;float:left'><img src=",sample.sample.files$path_pca[i]," />","\n"))
      ...
      output<-c(output,cat("<figure style='width:50%;float:right'><img src=",sample.sample.files$path_heatmap[i]," />","\n"))
...
no_null<-pca_heatmap.plot(analysis_type, DE)
...
summary.table<-function(comparison=comparison, analysis_type = analysis_type, DE = DE){
  ...
  for(i in 1:length(comparison)){
    ...
    for(j in 1:length(DE)){
      de.tab.edger <- fread(paste0("../results/",DE[j],"/",comparison[i],"/all/edgeR.tsv"),
                          sep  = "\t",header = T)
      de.tab.deseq <- fread(paste0("../results/",DE[j],"/",comparison[i],"/all/DESeq2.tsv"),
                          sep  = "\t",header = T)
...
volcano.ma.plots<-function(comparison=comparison, analysis_type = analysis_type, DE = DE){
  ...
  for(j in 1:length(DE)){
    volcano.files <- grep(grep(grep(list.files(path=paste0("../results/",DE[j],"/",comparison[1],"/all/"),
                               pattern = "volcano", full.names = T), pattern="Filt", value = T, invert = T),
                               pattern="edgeR", value = T, invert = T), pattern="ggplot2.png", value=T)
    ma.files <- grep(grep(grep(list.files(path=paste0("../results/",DE[j],"/",comparison[1],"/all/"),
                               pattern = "MAplot_", full.names = T), pattern="Filt", value = T, invert = T),
                               pattern="edgeR", value = T, invert = T), pattern="ggpubr.png", value=T)
  ...
  if(length(comparison)>1){
    for(i in 2:length(comparison)){
      output<-c(output,cat("#### ",comparison[i],"\n"))
      for(j in 1:length(DE)){
        volcano.files <- grep(grep(grep(list.files(path=paste0("../results/",DE[j],"/",comparison[i],"/all/"),
                               pattern = "volcano", full.names = T), pattern="Filt", value = T, invert = T),
                               pattern="edgeR", value = T, invert = T), pattern="ggplot2.png", value=T)
        ma.files <- grep(grep(grep(list.files(path=paste0("../results/",DE[j],"/",comparison[i],"/all/"),
                               pattern = "MAplot_", full.names = T), pattern="Filt", value = T, invert = T),
                               pattern="edgeR", value = T, invert = T), pattern="ggpubr.png", value=T)
...
top.de.plots<-function(comparison=comparison, analysis_type = analysis_type, DE = DE){
  output<-cat("#### ",comparison[1]," {.tabset}","\n")
  for(j in 1:length(DE)){
    top_de.files <- list.files(path=paste0("../results/",DE[j],"/",comparison[1],"/all/"),
  ...
  if(length(comparison)>1){
    for(i in 2:length(comparison)){
      output<-c(output,cat("#### ",comparison[i],"\n"))
      for(j in 1:length(DE)){
        top_de.files <- list.files(path=paste0("../results/",DE[j],"/",comparison[i],"/all/"),
                               pattern ="heatmap_selected_orderBaseMeanCluster.png", full.names = T)
