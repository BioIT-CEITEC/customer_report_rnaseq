...
DE <- "DE_feature_count" # feature_count is usually run by default
...
`r if(umi != "No UMI"){"- Samples contained 6bp long UMIs used for detection of PCR duplicates in library."}`
...
map.min <- round(min(tabl_mapread[,mapped]/1E6))
map.max <- round(max(tabl_mapread[,mapped]/1E6))
...
and the number of mapped reads varies between ~`r map.min`M for `r tabl_mapread[round(min(tabl_mapread[,mapped]/1E6)) == map.min,]$Sample` and ~`r map.max`M for `r tabl_mapread[round(max(tabl_mapread[,mapped]/1E6)) == map.max,]$Sample` (Table \@ref(tab:tabl-mapread), Figure \@ref(fig:star-map-perc) & \@ref(fig:star-map-count)).
...
`r if(config_json$display_fastq_screen_RNA[1] == TRUE){paste0("  - This can be due to less effective polyA selection procedure, but overall we still have a lot of usable reads and information to perform Differential expression analysis.")}`
...
- All samples had a `r intron.count` content of intronic reads ~`r picard.intron.aligned$intron`% counted over aligned bases as well as ~`r rseqc.intron$intron`% counted over reads which is expected for polyA selection library protocols
- All samples had a `r coding.count` content of mapped reads within coding regions ~`r picard.mrna.aligned$mrna`% counted over bases as well as ~`r rseqc.mrna$mrna`% counted over reads (Figure \@ref(fig:picard-map-loc-pct-align) & \@ref(fig:picard-map-loc-pct) & \@ref(fig:picard-map-loc-count) and \@ref(fig:rseqc-map-loc-pct) & \@ref(fig:rseqc-map-loc-count)).

#### Picard metrics {.tabset}

##### Percentages (over aligned bases)
...
plotly.picard.metric.perc.align
```

##### Percentages (over all bases)
...
- The analysis shows the strand-specificity of each library, but cannot be trusted with full confidence.
  - All the samples have ~`r rseqc.strand.spec[variable == "Sense", mean]`% of the forward-strand specificity, ~`r rseqc.strand.spec[variable == "Antisense", mean]`% of the reverse-strand specificity but ~`r rseqc.strand.spec[variable == "Undetermined", mean]`% mappings cannot be determined.
...
if(config_json$RSEM[1] == TRUE){
  ...
  rsem.assign.min <- round(min(rsem.assign[variable == "Unique", value]/1E6),0)
  rsem.assign.max <- round(max(rsem.assign[variable == "Unique", value]/1E6),0)
  rsem.assign.min.perc <- round(min(rsem.assign[variable == "Unique", percentage]),0)
  rsem.assign.max.perc <- round(max(rsem.assign[variable == "Unique", percentage]),0)
...
if(config_json$salmon_bam[1] == TRUE){
  salmonB.assign <- fread(paste0(MQC_DATA,"multiqc_salmon.txt"))
  salmonB.assign <- merge(config_json[, .(Sample = full_name, condition)], salmonB.assign, by="Sample")
  setorder(salmonB.assign, condition, Sample)

  salmonB.assign <- melt(salmonB.assign[, percent_aligned:=NULL], id.vars = c("Sample","condition","total_reads"))
  salmonB.assign <- salmonB.assign[, percentage := round(100*value/total_reads, 2), by = "Sample"]
  salmonB.assign <- salmonB.assign[, label:=variable]
  levels(salmonB.assign$label) <- c("Pseudoaligned","Not aligned")

  plot.salmonB.assign.count <- ggplot(salmonB.assign, aes(Sample, value, fill = label)) +
    geom_bar(stat = "identity" , width = 0.8, position = position_stack(reverse = TRUE)) +
  ...
  plotly.salmonB.assign.count <- ggplotly(plot.salmonB.assign.count, tooltip = "value") %>% layout(legend = list(orientation = 'h', y=-0.3))

  plot.salmonB.assign.pct <- ggplot(salmonB.assign, aes(Sample, percentage, fill = label)) +
    geom_bar(stat = "identity" , width = 0.8, position = position_stack(reverse = TRUE)) +
    scale_fill_manual(values = star.palette[c(1,5)], name = "") +
    coord_flip() +
    xlab("") +
  ...
  plotly.salmonB.assign.pct <- ggplotly(plot.salmonB.assign.pct, tooltip = "percentage") %>% layout(legend = list(orientation = 'h', y=-0.3))

}

if(config_json$salmon_fastq[1] == TRUE){
  salmonF.assign <- fread(paste0(MQC_DATA,"multiqc_salmon.txt"))
  salmonF.assign <- merge(config_json[, .(Sample = full_name, condition)], salmonF.assign, by="Sample")
  setorder(salmonF.assign, condition, Sample)

  salmonF.assign <- melt(salmonF.assign[, percent_aligned:=NULL], id.vars = c("Sample","condition","total_reads"))
  salmonF.assign <- salmonF.assign[, percentage := round(100*value/total_reads, 2), by = "Sample"]
  salmonF.assign <- salmonF.assign[, label:=variable]
  levels(salmonF.assign$label) <- c("Pseudoaligned","Not aligned")

  plot.salmonF.assign.count <- ggplot(salmonF.assign, aes(Sample, value, fill = label)) +
    geom_bar(stat = "identity" , width = 0.8, position = position_stack(reverse = TRUE)) +
    scale_fill_manual(values = star.palette[c(1,5)], name = "") +
    coord_flip() +
    xlab("") +
  ...
  plotly.salmonF.assign.count <- ggplotly(plot.salmonF.assign.count, tooltip = "value") %>% layout(legend = list(orientation = 'h', y=-0.3))

  plot.salmonF.assign.pct <- ggplot(salmonF.assign, aes(Sample, percentage, fill = label)) +
    geom_bar(stat = "identity" , width = 0.8, position = position_stack(reverse = TRUE)) +
    scale_fill_manual(values = star.palette[c(1,5)], name = "") +
    coord_flip() +
    xlab("") +
  ...
  plotly.salmonF.assign.pct <- ggplotly(plot.salmonF.assign.pct, tooltip = "percentage") %>% layout(legend = list(orientation = 'h', y=-0.3))
...
if(config_json$kallisto[1] == TRUE){
  ...
  kallisto.assign <- melt(kallisto.assign[, percent_aligned:=NULL], id.vars = c("Sample","condition","total_reads"))
...
`r if(config_json$feature_count[1] == TRUE){paste0("- Read count assignment from **featureCounts** for RNA-Seq is quite high (between ",fc.assign.min.perc,"% and ",fc.assign.max.perc",%, calculated only from uniquely mapped reads).")}`
...
`r if(config_json$feature_count[1] == TRUE){paste0("  - Read assignment rates are summarized in Table \\@ref(tab:fc-assign-tab) and Figure \\@ref(fig:plotly-fc-assign-pct) & \\@ref(fig:plotly-fc-assign-count).")}`
...
`r if(config_json$RSEM[1] == TRUE){paste0("- Read count assignment from **RSEM** for RNA-Seq is very high (between ",rsem.assign.min.perc," % and ",rsem.assign.max.perc," %).")}`
`r if(config_json$RSEM[1] == TRUE){paste0("  - Number of assigned reads is on a border of the recommended number of reads (~",rsem.assign.min,"-",rsem.assign.max,"M).")}`
`r if(config_json$RSEM[1] == TRUE){paste0("  - Read assignment rates are summarized in Table \\@ref(tab:rsem-assign-tab) and Figure \\@ref(fig:rsem-assign).")}`
...
```{r plot-rsem-assign-pct, results='asis', echo=FALSE, fig.cap="RSEM: Assignments.", fig.align="center", out.width="90%", fig.show="hold", dev="svg", fig.width=7, fig.height = custom_height1}
if(config_json$RSEM[1] == TRUE){plot.rsem.assign.pct}
```
...
```{r plot-kallisto-assign-pct, results='asis', echo=FALSE, fig.cap="Kallisto: Assignments.", fig.align="center", out.width="90%", fig.show="hold", dev="svg", fig.width=7, fig.height = custom_height1}
if(config_json$kallisto[1] == TRUE){plot.kallisto.assign.pct}
```
...
if(config_json$conditions_to_compare[1] == "all"){
  ...
  sample.sample.files[, path:=ifelse(file.exists(paste0("../results/",DE,"/",comp,"/all/sample_to_sample_PCA_batch2.svg")), paste0("../results/",DE,"/",comp,"/all/sample_to_sample_PCA_batch2.svg"), paste0("../results/",DE,"/",comp,"/all/sample_to_sample_PCA2.svg"))]
}else{
