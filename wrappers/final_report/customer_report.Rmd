...
custom_height1 <- ifelse(num_samples < 50, default_height, 10)
...
custom_height2 <- ifelse(paired == "paired-end" & num_samples >= 50, 15, custom_height1)
```
...
### IGV
- You can upload the provided aligned bam files.
...
if(length(ppnc.files)>1){
  for(i in 2:length(ppnc.files)){
    cat("<div align='center'><img src=",ppnc.files$path[i]," width='80%'/></div>")
    #cat("![](",ppnc.files[i],")")
    cat('\n\n')
  }
}
...
if(config_json$conditions_to_compare[1] == "all"){
  ...
  heatmap.sample.files[, path:=ifelse(file.exists(paste0("../results/",DE,"/",comp,"/all/heatmaps_samples_batch.pdf-0.png")), paste0("../results/",DE,"/",comp,"/all/heatmaps_samples_batch.pdf-0.png"), paste0("../results/",DE,"/",comp,"/all/heatmaps_samples.pdf-0.png"))]
}else{
  heatmap.sample.files <- data.table(comp = comparison)
  heatmap.sample.files[, path:=ifelse(file.exists(paste0("../results/",DE,"/",comp,"/all/heatmaps_samples_batch.pdf-0.png")), paste0("../results/",DE,"/",comparison,"/all/heatmaps_samples_batch.pdf-0.png"), paste0("../results/",DE,"/",comp,"/all/heatmaps_samples.pdf-0.png"))]
}
...
volcano.files <- grep(grep(grep(list.files(path=paste0("../results/",DE,"/",comparison,"/all/"), pattern = "volcano", full.names = T), pattern="Filt", value = T, invert = T), pattern="edgeR", value = T, invert = T), pattern="ggplot2.pdf.png", value=T)

if(length(volcano.files)>1){
  for(i in 2:length(volcano.files)){
    cat("<div align='center'><img src=",volcano.files$path[i]," width='80%'/></div>")
    #cat("![](",volcano.files[i],")")
    cat('\n\n')
  }
}
...
ma.files <- grep(grep(grep(list.files(path=paste0("../results/",DE,"/",comparison,"/all/"), pattern = "MAplot_", full.names = T), pattern="Filt", value = T, invert = T), pattern="edgeR", value = T, invert = T), pattern="ggpubr.pdf.png", value=T)

if(length(ma.files)>1){
  for(i in 2:length(ma.files)){
    cat("<div align='center'><img src=",ma.files$path[i]," width='80%'/></div>")
    #cat("![](",ma.files[i],")")
    cat('\n\n')
  }
}
...
```{r de-top-de, fig.cap = paste0("Heatmap of maximum top 20 differentially expressed genes (DESeq2 results with independent filtering, adj.p-value < 0.05, logFC >= +/-1)."), echo=FALSE, out.height="80%", fig.align="center", results="asis", fig.show="hold", dev="svg"}

top_de.files <- list.files(path=paste0("../results/",DE,"/",comparison,"/all/"),pattern ="heatmap_selected_orderBaseMeanCluster.pdf.png", full.names = T)
if(length(top_de.files)>0){
  if(length(top_de.files)>1){
    for(i in 2:length(top_de.files)){
      cat("<div align='center'><img src=",top_de.files$path[i]," width='80%'/></div>")
      #cat("![](",top_de.files[i],")")
      cat('\n\n')
    }
  }
