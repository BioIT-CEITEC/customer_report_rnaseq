...
analysis_type <- gsub("salmon_align","Salmon (alignment mode)",analysis_type)
...
norm.plot<-function(analysis_type = analysis_type, DE = DE){
  ...
  for(j in 1:length(analysis_type)){
    ...
    if(config_json$keep_not_compared_samples_for_normalization[1] == TRUE){
      ppnc.files <- list.files(path=paste0("../",DE[j],"/all_condition_results/report_data/"),pattern ="pre_post_norm_counts.svg", full.names = T)
    }else{
      ppnc.files <- list.files(path=paste0("../",DE[j],"/",comparison,"/detail_results/report_data/"),pattern ="pre_post_norm_counts.svg", full.names = T)
    }
...
plot.dt <- function(analysis.type = analysis_type, comp.folder = comparison, path.prefix = "../", path.suffix = "/report_data/", pattern1 = "volcanoplot.+png", pattern2 = "volcanoplot.+png"){
  ...
  for(i in 1:length(comparison)){
    ...
    for(j in 1:length(DE)){
      de.tab.edger <- fread(paste0("../",DE[j],"/",comparison[i],"/edgeR/edgeR.tsv"),
                          sep  = "\t",header = T)
      de.tab.deseq <- fread(paste0("../",DE[j],"/",comparison[i],"/detail_results/full_DESeq2.tsv"),
                          sep  = "\t",header = T)
